...
class ViewController: UIViewController {
    ...
    func editImage(_ image: UIImage, editModel: ZLEditImageModel?) {
        ...
        ZLImageEditorConfiguration.default().editImageTools=[.clip, .imageSticker]
        ZLImageEditorConfiguration.default().strikerIconImage = UIImage(named: "plant_icon")
...
//        ZLImageEditorConfiguration.default().filters = [.normal, .apply1977, ZLFilter(name: "Custom", applier: CustomFilter.hazeRemovalFilter)]
        ...
        let viewcontroller=ZLEditImageViewController.showEditImageVC(parentVC: self, image: image, editModel: editModel) { [weak self] (resImage, editModel) in
            self?.resultImageView.image = resImage
            self?.resultImageEditModel = editModel
        }
        if viewcontroller is ZLEditImageViewController {
            let v:ZLEditImageViewController=viewcontroller as! ZLEditImageViewController
//            v.doneBtnClick()
            v.hideDoneBtn()
            v.hideCancelBtn()
            v.imageStickerBtnClick()
        }
    }
